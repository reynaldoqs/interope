 ...
 <v-container align-center class="my-container my-buttons">
         <v-layout wrap>
          <v-flex sm12  md6 lg4 align-content-center>
            <v-btn outline round color="primary" @click="$router.push('/admin/rutas/create')">Agregar Ruta</v-btn>
          </v-flex>
          <v-flex sm12  md6 lg4 align-content-center>
            <v-text-field
...
import { getRutas, searchRuta } from '@/services/rutasService';
...
export default {
  data(){
    return{
      ...
      headers:[
        {
          text: 'Nombre de la ruta', align: 'left', sortable: false,
          value: 'nombre'
        },
 ...
 mounted() {
    ...
    getRutas().then(data => {
      ...
      this.pages=data.data.totalRutas;
      this.rutas = data.data.rutas;
      this.loading = false;
    }).catch(err => console.log(err));
  },
  methods:{
    ...mapActions(["notification"]),//quitar, solo para pruebas
    ...
    editItem(id){
      this.$router.push(`/admin/rutas/edit/${id}`)
    },
    detailItem(id){
      this.$router.push(`/admin/rutas/detail/${id}`)
    }
    ,
    updatedPagination(){
      this.loading = true;
      getRutas(10,this.pagination.page).then(data => {
      this.pagination.page= data.data.pagina;
